...
//     user := createTestUser("like_user", "likeuser", "like@example.com")
//     require.NoError(t, service.CreateUser(ctx, user))

//     // Generate valid UUID for track
//     trackID := uuid.NewString()

//     err := service.LikeTrack(ctx, "like_user", trackID)

//     assert.NoError(t, err)
// }

// func TestLikeTrack_Duplicate(t *testing.T) {
//     db := GetTestDB(t)
//     defer db.Close()
...
//     user := createTestUser("dup_like_user", "duplikeuser", "duplike@example.com")
//     require.NoError(t, service.CreateUser(ctx, user))

//     trackID := uuid.NewString()
//     require.NoError(t, service.LikeTrack(ctx, "dup_like_user", trackID))

//     err := service.LikeTrack(ctx, "dup_like_user", trackID)

//     assert.NoError(t, err)
// }

func TestLikeTrack_InvalidInput_EmptyUserID(t *testing.T) {
    db := GetTestDB(t)
    defer db.Close()
    ...
    err := service.LikeTrack(ctx, "", "track_123")

    assert.ErrorIs(t, err, users.ErrInvalidInput)
}

func TestLikeTrack_InvalidInput_EmptyTrackID(t *testing.T) {
    db := GetTestDB(t)
    defer db.Close()
    ...
    err := service.LikeTrack(ctx, "some_user", "")

    assert.ErrorIs(t, err, users.ErrInvalidInput)
}

// func TestUnlikeTrack_Success(t *testing.T) {
//     db := GetTestDB(t)
//     defer db.Close()
...
//     user := createTestUser("unlike_user", "unlikeuser", "unlike@example.com")
//     require.NoError(t, service.CreateUser(ctx, user))

//     trackID := uuid.NewString()
//     require.NoError(t, service.LikeTrack(ctx, "unlike_user", trackID))

//     err := service.UnlikeTrack(ctx, "unlike_user", trackID)

//     assert.NoError(t, err)
// }

func TestUnlikeTrack_NotLiked(t *testing.T) {
    db := GetTestDB(t)
    defer db.Close()
    ...
    user := createTestUser("notliked_user", "notlikeduser", "notliked@example.com")
    require.NoError(t, service.CreateUser(ctx, user))

    trackID := uuid.NewString()
    err := service.UnlikeTrack(ctx, "notliked_user", trackID)

    assert.Error(t, err)
}

func TestUnlikeTrack_InvalidInput(t *testing.T) {
    db := GetTestDB(t)
    defer db.Close()
    ...
    err := service.UnlikeTrack(ctx, "", "track_123")
...
//     user := createTestUser("liked_user", "likeduser", "liked@example.com")
//     require.NoError(t, service.CreateUser(ctx, user))
//     require.NoError(t, service.LikeTrack(ctx, "liked_user", uuid.NewString()))
//     require.NoError(t, service.LikeTrack(ctx, "liked_user", uuid.NewString()))
//     require.NoError(t, service.LikeTrack(ctx, "liked_user", uuid.NewString()))

//     tracks, total, err := service.GetLikedTracks(ctx, "liked_user", 10, 0)
...
func TestGetLikedTracks_Empty(t *testing.T) {
    db := GetTestDB(t)
    defer db.Close()
    ...
    user := createTestUser("noliked_user", "nolikeduser", "noliked@example.com")
    require.NoError(t, service.CreateUser(ctx, user))

    tracks, total, err := service.GetLikedTracks(ctx, "noliked_user", 10, 0)
...
// func TestGetLikedTracks_Pagination(t *testing.T) {
//     db := GetTestDB(t)
//     defer db.Close()
...
//     user := createTestUser("paginate_liked_user", "paginatelikeduser", "paginateliked@example.com")
//     require.NoError(t, service.CreateUser(ctx, user))

//     for i := 0; i < 5; i++ {
//         require.NoError(t, service.LikeTrack(ctx, "paginate_liked_user", uuid.NewString()))
//     }

//     tracks, total, err := service.GetLikedTracks(ctx, "paginate_liked_user", 2, 0)
...
func TestGetLikedTracks_InvalidInput(t *testing.T) {
    db := GetTestDB(t)
    defer db.Close()
    ...
    _, _, err := service.GetLikedTracks(ctx, "", 10, 0)
...
// func TestGetUserStats_WithData(t *testing.T) {
//     db := GetTestDB(t)
//     defer db.Close()
...
//     user1 := createTestUser("stats_user_data", "statsuserdata", "statsdata@example.com")
//     user2 := createTestUser("stats_follower", "statsfollower", "statsfollower@example.com")
//     user3 := createTestUser("stats_following", "statsfollowing", "statsfollowing@example.com")
//     require.NoError(t, service.CreateUser(ctx, user1))
//     require.NoError(t, service.CreateUser(ctx, user2))
//     require.NoError(t, service.CreateUser(ctx, user3))

//     require.NoError(t, service.LikeTrack(ctx, "stats_user_data", uuid.NewString()))
//     require.NoError(t, service.LikeTrack(ctx, "stats_user_data", uuid.NewString()))

//     require.NoError(t, service.Follow(ctx, "stats_follower", "stats_user_data"))
//     require.NoError(t, service.Follow(ctx, "stats_user_data", "stats_following"))

//     stats, err := service.GetUserStats(ctx, "stats_user_data")

//     assert.NoError(t, err)
//     assert.NotNil(t, stats)
//     assert.Equal(t, 2, stats.LikedTracks)
//     assert.Equal(t, 1, stats.FollowersCount)
